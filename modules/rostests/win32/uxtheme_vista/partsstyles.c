...
static void GetExpandoSizes(HWND hWnd)
{
    ...
    if (error)
    {
        const LPWSTR message = L"Could not TDLG_EXPANDOBUTTON width or height";
        ...
        case WM_PAINT:
            {
                ...
                if (FAILED(hr))
                {
                    const LPWSTR message = L"GetThemeFont(TEXT_MAININSTRUCTION) failed";
                    PostMessageW(hWnd, WM_EXIT_THEME_ERROR, (WPARAM)message, 0);
                    EndPaint(hWnd, &ps);
                    break;
                }
                ...
                if (FAILED(hr))
                {
                    const LPWSTR message = L"Could not get TEXT_MAININSTRUCTION text color.";
                    PostMessageW(hWnd, WM_EXIT_THEME_ERROR, (WPARAM)message, 0);
                    EndPaint(hWnd, &ps);
                    break;
                }
        ...
        case WM_EXIT_THEME_ERROR:
            {
                ...
                MessageBoxW(hWnd, message, L"Fatal Theme Error", MB_ICONERROR | MB_OK);
                DestroyWindow(hWnd);
            }
            break;
...
wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpwszCmdLine, int nCmdShow)
{
    ...
    if (!IsAppThemed())
    {
        MessageBox(HWND_DESKTOP, L"This application requires visual styles to be enabled.", L"Error", MB_ICONERROR | MB_OK);
        return 0;
    }
